...
region_alloc(struct Env *e, void *va, size_t len)
{
	...
	struct PageInfo * pp;
	for (int i=0; i<region_size/PGSIZE; i++){
		pp = page_alloc(0);
		page_insert(e->env_pgdir, pp, ROUNDDOWN(va, PGSIZE)+i*PGSIZE, PTE_U | PTE_W);
	}
}
...
load_icode(struct Env *e, uint8_t *binary)
{
	...
	for (; ph < eph; ph++){
		// p_va is the load address of this segment (as well
