...
mem_init(void)
{
	...
	// Your code goes here:
	int pages_size = ROUNDUP((void*) (pages+npages), PGSIZE)-(void*)pages;
	boot_map_region(kern_pgdir, UPAGES, pages_size, PADDR(pages), PTE_U | PTE_P);
	boot_map_region(kern_pgdir, (uint32_t)pages, pages_size, PADDR(pages), PTE_W);
	...
	// LAB 3: Your code here.
	int envs_size = ROUNDUP((void*) (envs+NENV), PGSIZE)-(void*)envs;
	boot_map_region(kern_pgdir, UENVS, envs_size, PADDR(envs), PTE_U | PTE_P);
	boot_map_region(kern_pgdir, (uint32_t)envs, envs_size, PADDR(envs), PTE_W);
	...
	// boot_map_region(kern_pgdir, KSTACKTOP-PTSIZE, PTSIZE-KSTKSIZE, PADDR(bootstacktop)-PTSIZE, 0);
...
page_init(void)
{
	...
	int npages_free = PADDR(boot_alloc(0)) / PGSIZE;
	for (i = npages_free; i < npages; i++){
		pages[i].pp_ref = 0;
		pages[i].pp_link = page_free_list;
		page_free_list = &pages[i];
	}
...
page_free(struct PageInfo *pp)
{
	...
	if (pp->pp_ref){
		panic("nonzero pp_ref in page free");
	}
	if (pp->pp_link){
		panic("corrupted pp_link, NULL is expected");
	}
...
{
	// LAB 3: Your code here.
	void *cur=ROUNDDOWN((void*)va, PGSIZE), *end=ROUNDUP((void*)va+len, PGSIZE);
	pte_t *pte;
	for (; cur < end; cur+=PGSIZE){
		pte = pgdir_walk(env->env_pgdir, cur, 0);
